...
def main(kernel_kind="exp", tau_weeks=4):
    ...
    eps_grid = [0.0, 0.02, 0.05, 0.10, 0.15, 0.20, 0.35, 0.50, 0.55, 0.60]
    best_eps = {}
    all_eps_results = []
    ...
    for h in HORIZONS:
        ...
        cal_train = cal_all[cal_all["year"].isin([2017, 2018])].copy()
        cal_val   = cal_all[cal_all["year"].isin([2019, 2020])].copy()
        ...
        for eps in eps_grid:
            ...
            res = tuning_objective_combined_band(
                    val_out,
                    alpha=ALPHA,
            ...
            rows.append((
                    ...
                    res["coverage"],
                    res["median_neff"],
                    res["coverage_penalty"],
                    res["neff_penalty"],
                    eps
                    ))
            all_eps_results.append({"Horizon": h, "interval_score": res["interval_score"], "coverage": res["coverage"],
                                 "Objective": res["objective"], "Median_neff": res["median_neff"], "epsilon": eps})
            
        rows.sort(key=lambda x: x[1])

        for row in rows:
            print(f"tau={row[0]} | obj={row[1]:.3f} | IS={row[2]:.3f} | "
            f"cov={row[3]:.3f} | med_n_eff={row[4]:.1f} | "
            f"cov_pen={row[5]:.3f} | neff_pen={row[6]:.3f}")
    ...
    for h in HORIZONS:
        ...
        out_test["kernel_kind"] = kernel_kind
        out_test["tau_weeks"] = tau_weeks
        ...
        out_test["Epsilon"] = best_eps[h]
        out_test = out_test[KEEP_COLUMNS]
