...
def run_region_season_conformal(
    ...
    *,
    ...
    for h in sorted(cal_files):
        ...
        q_table = (
            cal.groupby([region_col, "Season"], observed=False)["score"]
            .quantile(1 - alpha)
            .reset_index()
            .rename(columns={"score": "q_hat"})
        )

        fallback_q = cal["score"].quantile(1 - alpha)

        test2 = test.merge(q_table, how="left", on=[region_col, "Season"])
        test2["q_hat"] = test2["q_hat"].fillna(fallback_q)
    ...
    return ConformalResult(
        method="region_season_stratified",
